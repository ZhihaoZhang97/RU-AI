...
def load_and_transform_image(image_path, transform):
    ...
    if len(image_array.shape) != 3 or (len(image_array.shape) == 3 and image_array.shape[-1] != 3):
        image_array_three_channels = np.stack((image_array,) * 3, axis=-1)
        image = Image.fromarray(image_array_three_channels)
